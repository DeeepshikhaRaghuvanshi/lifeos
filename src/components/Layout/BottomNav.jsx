...
export default function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 pt-3 pb-[calc(env(safe-area-inset-bottom,12px)+16px)] z-[100] flex justify-between items-center shadow-[0_-8px_20px_rgba(0,0,0,0.04)]">
      {navItems.map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
